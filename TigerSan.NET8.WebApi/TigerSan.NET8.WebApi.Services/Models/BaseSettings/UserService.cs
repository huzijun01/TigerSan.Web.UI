    ...
    public class UserService : IUserService
    {
        ...
        public MyActionResult<CaptchaData> GetCaptcha()
        {
            try
            {
                ...
                captcha.Captcha = CaptchaHelper.GenerateImageBytes(code);
                if (captcha.Captcha == null) return MyResults<CaptchaData>.CaptchaGenerationFailed;
                CaptchaCache.InitCaptcha(captcha.Id, code);
                return MyResults<CaptchaData>.Success(null, captcha);
            }
            catch (Exception e)
            {
                return MyResults<CaptchaData>.Error(LogHelper.Instance.Error(e.GetMessage()));
            }
        }
        #endregion
