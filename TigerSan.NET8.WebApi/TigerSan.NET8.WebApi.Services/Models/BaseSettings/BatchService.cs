...
namespace TigerSan.NET8.WebApi.Services.Models
{
    public class BatchService : IdServiceBase<BatchEntity>, IBatchService
    {
        ...
        public async Task<MyActionResult<CompanyEntity>> GetCompany(long id)
        {
            ...
            {
                return MyResults<CompanyEntity>.Error(LogHelper.Instance.Error(e.GetMessage()));
            }
        }
        #endregion
        ...
        public async Task<MyActionResult<Dictionary<long, CompanyEntity>>> GetCompanyDict(List<long> ids)
        {
            ...
            {
                return MyResults<Dictionary<long, CompanyEntity>>.Error(LogHelper.Instance.Error(e.GetMessage()));
            }
        }
        #endregion
        #endregion [查]
        ...
        public override async Task<MyActionResult<BatchEntity>> Add(BatchEntity entity, bool isBeginTransaction = true)
        {
            ...
            try
            {
                ...
                return MyResults<BatchEntity>.Success(null, entity);
            }
            catch (Exception e)
            {
                if (transaction != null) await transaction.RollbackAsync(); // 回滚所有操作
                return MyResults<BatchEntity>.Error(LogHelper.Instance.Error(e.GetMessage()));
            }
        }
        #endregion
