...
import { GetTableModel } from './StationBindingTable'
import { stationBindingHelper, StationBindingDto } from '@/models'

export class StationBindingPageModel {
    //#region 【Fields】
    _tag?: bigint
    _station?: bigint
    /** 分页器 */
    ...
    /** “标签ID”项目配置 */
    readonly configTagId: FormItemConfig<StationBindingDto, IdName> = {
        _propName: 'tagId',
    ...
    /** “资产ID”项目配置 */
    readonly configStationId: FormItemConfig<StationBindingDto, string> = {
        _propName: 'stationId',
        PropText: Texts.StationId,
        IsEquired: true,
        Target: ref(),
        _isVerifyOk: source => {
            return Verify.IsNotUndefinedOrEmpty(source.stationId)
        }
    }

    /** “增”源数据获取方法 */
    readonly AddGetSource = () => new StationBindingDto()

    /** 表单配置 */
    readonly configStationBindingPageModel: FormConfig<StationBindingDto> = {
    ...
    /** 表单模型 */
    readonly form = new FormModel(this.configStationBindingPageModel)
    ...
    readonly Refresh = async () => {
        try {
            ...
            if (!this._tag && !this._station) {
                console.warn('The _tag and _station is undefined!')
                return
            }

            this.pagination.Count.value = await stationBindingHelper.GetCount({
                tag: this._tag,
                station: this._station,
            })

            await stationBindingHelper.GetList({
    ...
    /** 增 */
    readonly Add = async () => {
        this.form.Title.value = TextModel.GetText('Add StationBinding', '新增绑定记录')

        this.form._getSource = this.AddGetSource

        this.form._onSubmitAsync = async source => {
            const res = await stationBindingHelper.Add(source)
            await this.Refresh()
    ...
    /** 改 */
    readonly Edit = async () => {
        this.form.Title.value = TextModel.GetText('Edit StationBinding', '修改绑定记录')
        ...
        this.form._onSubmitAsync = async source => {
            const res = await stationBindingHelper.Edit(source)
            await this.Refresh()
    ...
    readonly DeleteRowData = async (state: DialogState) => {
        ...
        try {
            ...
            await stationBindingHelper.DeleteRange(rowDatas).then(res => {
                this.Refresh()
