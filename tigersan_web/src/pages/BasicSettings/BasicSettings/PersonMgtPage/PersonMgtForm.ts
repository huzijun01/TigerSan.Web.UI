import { ref } from 'vue'
import { Colors, DialogHelper, Verify, ObjectHelper, DialogMode, DialogState, FormModel, FormConfig, FormItemConfig, ArrayHelper, BigintHelper, PasswordModel, SearchModel, GetSubmitResult, IdNameModel, MyActionResult, loading, PaginationModel } from '@/0_tigersan_ui/tigerui'
...
export class PersonMgtForm {
    ...
    constructor() {
        ...
        this.selectDepartment._getItemsAsync = async () => this.selectCompany.Value.value ? await personHelper.GetBelongDepartmentListAsync(this.selectCompany.Value.value?.id) : []
        this.selectRole._getItemsAsync = async () => this.selectDepartment.Value.value ? await personHelper.GetBelongRoleListAsync(this.selectDepartment.Value.value?.id) : []
        ...
        this.selectRoleForm._getItemsAsync = async () => this.selectDepartmentForm.Value.value ? await roleHelper.SelectIdNameByDepartment(this.selectDepartmentForm.Value.value?.id) : []
        // 更新:
