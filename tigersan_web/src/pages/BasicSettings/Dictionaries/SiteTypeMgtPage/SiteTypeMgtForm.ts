...
import { SiteTypeModel, siteTypeMgtHelper } from '@/models'
...
/** 改 */
async function Edit() {
    siteTypeMgtForm.Title.value = '修改部门'

    siteTypeMgtForm._getSource = () => {
        const rowData = siteTypeMgtTable.SelectedRowDatas.value[0]

        if (!rowData) {
            console.warn('The rowData is undefined!')
            return new SiteTypeModel()
        }
    ...
    siteTypeMgtForm._onSubmitAsync = async source => {
        const res = await siteTypeMgtHelper.Edit(source)
        await Refresh()
