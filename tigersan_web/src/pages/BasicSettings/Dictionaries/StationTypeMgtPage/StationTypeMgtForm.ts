...
/** 改 */
async function Edit() {
    stationTypeMgtForm.Title.value = '修改部门'

    stationTypeMgtForm._getSource = () => {
        const rowData = stationTypeMgtTable.SelectedRowDatas.value[0]

        if (!rowData) {
            console.warn('The rowData is undefined!')
            return new StationTypeModel()
        }
    ...
    stationTypeMgtForm._onSubmitAsync = async source => {
        const res = await stationTypeMgtHelper.Edit(source)
        await Refresh()
