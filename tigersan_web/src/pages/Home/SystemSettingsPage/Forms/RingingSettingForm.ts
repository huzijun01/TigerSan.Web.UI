...
/** “单次响铃时长”项目配置 */
const configringingTime: FormItemConfig = {
...
/** “快、响铃”项目配置 */
const configRinging_Fast_On: FormItemConfig = {
    _propName: 'Ringing_Fast_On',
    PropText: '响铃',
    Target: ref<unknown>(),
    _isVerifyOk: (source) => {
        var ringingSetting = source as ringingSettingModel
        return Verify.IsWithinRange(ringingSetting.Ringing_Fast_On, 200, 65000)
    }
}

/** “快、空闲”项目配置 */
const configRinging_Fast_Off: FormItemConfig = {
    _propName: 'Ringing_Fast_Off',
    PropText: '空闲',
    Target: ref<unknown>(),
    _isVerifyOk: (source) => {
        var ringingSetting = source as ringingSettingModel
        return Verify.IsWithinRange(ringingSetting.Ringing_Fast_Off, 200, 65000)
    }
}

/** “较快、响铃”项目配置 */
const configRinging_Faster_On: FormItemConfig = {
    _propName: 'Ringing_Faster_On',
    PropText: '响铃',
    Target: ref<unknown>(),
    _isVerifyOk: (source) => {
        var ringingSetting = source as ringingSettingModel
        return Verify.IsWithinRange(ringingSetting.Ringing_Faster_On, 200, 65000)
    }
}

/** “较快、空闲”项目配置 */
const configRinging_Faster_Off: FormItemConfig = {
    _propName: 'Ringing_Faster_Off',
    PropText: '空闲',
    Target: ref<unknown>(),
    _isVerifyOk: (source) => {
        var ringingSetting = source as ringingSettingModel
        return Verify.IsWithinRange(ringingSetting.Ringing_Faster_Off, 200, 65000)
    }
}

/** “中、响铃”项目配置 */
const configRinging_Medium_On: FormItemConfig = {
    _propName: 'Ringing_Medium_On',
    PropText: '响铃',
    Target: ref<unknown>(),
    _isVerifyOk: (source) => {
        var ringingSetting = source as ringingSettingModel
        return Verify.IsWithinRange(ringingSetting.Ringing_Medium_On, 200, 65000)
    }
}

/** “中、空闲”项目配置 */
const configRinging_Medium_Off: FormItemConfig = {
    _propName: 'Ringing_Medium_Off',
    PropText: '空闲',
    Target: ref<unknown>(),
    _isVerifyOk: (source) => {
        var ringingSetting = source as ringingSettingModel
        return Verify.IsWithinRange(ringingSetting.Ringing_Medium_Off, 200, 65000)
    }
}

/** “较慢、响铃”项目配置 */
const configRinging_Slower_On: FormItemConfig = {
    _propName: 'Ringing_Slower_On',
    PropText: '响铃',
    Target: ref<unknown>(),
    _isVerifyOk: (source) => {
        var ringingSetting = source as ringingSettingModel
        return Verify.IsWithinRange(ringingSetting.Ringing_Slower_On, 200, 65000)
    }
}

/** “较慢、空闲”项目配置 */
const configRinging_Slower_Off: FormItemConfig = {
    _propName: 'Ringing_Slower_Off',
    PropText: '空闲',
    Target: ref<unknown>(),
    _isVerifyOk: (source) => {
        var ringingSetting = source as ringingSettingModel
        return Verify.IsWithinRange(ringingSetting.Ringing_Slower_Off, 200, 65000)
    }
}
...
/** “慢、响铃”项目配置 */
const configRinging_Slow_On: FormItemConfig = {
    _propName: 'Ringing_Slow_On',
    PropText: '响铃',
    Target: ref<unknown>(),
    _isVerifyOk: (source) => {
        var ringingSetting = source as ringingSettingModel
        return Verify.IsWithinRange(ringingSetting.Ringing_Slow_On, 200, 65000)
    }
}

/** “慢、空闲”项目配置 */
const configRinging_Slow_Off: FormItemConfig = {
    _propName: 'Ringing_Slow_Off',
    PropText: '空闲',
    Target: ref<unknown>(),
    _isVerifyOk: (source) => {
        var ringingSetting = source as ringingSettingModel
        return Verify.IsWithinRange(ringingSetting.Ringing_Slow_Off, 200, 65000)
    }
}

/** “响铃设置”表单配置 */
let configringingSettingForm: FormConfig = {
    CancelText: '取消',
    SubmitText: '确定',
